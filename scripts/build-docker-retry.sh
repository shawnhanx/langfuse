...
# Docker build retry script for web
...
DOCKER_BUILD_CMD="docker build --platform linux/amd64 --no-cache -f web/Dockerfile -t ${LOCAL_IMAGE} ."

ATTEMPT=1

echo "========================================"
echo "Starting Docker build retry loop (Web)"
...
while true; do
  ...
  if eval "${DOCKER_BUILD_CMD}"; then
    ...
    if docker images | grep -q "langfuse-web.*${IMAGE_TAG}"; then
